...
namespace alloc
{
    ...
    size_t add_region(void* buf, size_t size)
    {
        size &= ~7;

        auto* block = new (buf) megablock_header { nullptr, nullptr };
        new (block + 1) block_header {
            (size - sizeof(megablock_header) - sizeof(block_header)) | 1,
            nullptr, 
            nullptr
        };
        if(root == nullptr)
    ...
    void* malloc(size_t size)
    {
        ...
        while(current_megablock)
        {
            ...
            while(hdr)
            {
                ...
                {
                    size_t bsize = hdr->size & ~1;
                    if(bsize - size >= sizeof(block_header))
                    {
                        auto* next_block = new ((uint8_t*)hdr + sizeof(block_header) + size) block_header {
                            (hdr->size - size - sizeof(block_header)),
                            hdr->next,
                            hdr
                        };
